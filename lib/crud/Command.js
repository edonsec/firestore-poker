...
class CrudCommand {
	...
	getCommands() { 
		return {
			create: {
				...
				options: [{label:'generate', description:'Generate a random id'}],
				action: (params, options) => this.create(params.collection, params.reference, params.json_data, options.generate)
			},
			read: {
	...
	async create(_collection, reference, data, generate) {
		...
		if ( generate) {
			...
			return await setDoc(doc(this.db, _collection, reference), JSON.parse(data))
				.then((r) => console.log(Colours.green("Document with reference: " + reference + " created")))
				.catch((e) => console.log(Colours.red(e.message)));
